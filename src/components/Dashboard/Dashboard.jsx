...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      if (!user?.uid) return;
      try {
        const myTasksQ = query(collection(db, 'tasks'), where('ownerUid', '==', user.uid));
        const myTasksSnap = await getDocs(myTasksQ);

        const browseableQ = query(collection(db, 'tasks'));
        const browseableSnap = await getDocs(browseableQ);

        const myBidsQ = query(collectionGroup(db, 'bids'), where('bidderUid', '==', user.uid));
        const myBidsSnap = await getDocs(myBidsQ);

        setStats({
          added: myTasksSnap.size,
          browseable: browseableSnap.size,
          posted: myTasksSnap.size,
          featured: 40,
        });
      } catch (err) {
