...
import { collection, collectionGroup, deleteDoc, doc, onSnapshot, query, where } from 'firebase/firestore';
...
const MyPostedTasks = () => {
  ...
  // Single collectionGroup listener for all bids owned by this user
  useEffect(() => {
    if (!user?.uid) return;
    const q = query(collectionGroup(db, 'bids'), where('taskOwnerUid', '==', user.uid));
    const unsub = onSnapshot(q, (snap) => {
      const counts = {};
      snap.forEach(d => {
        const data = d.data();
  ...
  return (
    <div className="max-w-6xl mx-auto mt-15 min-h-screen px-4 py-8">
      <title>My Posted Tasks || Lance Wave</title>
      ...
      <div className="md:hidden space-y-4">
        {tasks.map((task) => (
          ...
          >
            ...
            <p><strong>Bids:</strong> {bidCounts[task._id] || 0}</p>
